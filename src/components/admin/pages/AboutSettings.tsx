...
const AboutSettings: React.FC = () => {
    ...
    return (
        <div className="p-4 max-w-4xl mx-auto">
            <h2 className="text-2xl font-bold mb-8">Paramètres de la page À propos</h2>
            {/* Texte principal */}
            <form onSubmit={async e => { e.preventDefault(); await supabase.from('settings').upsert({ key: 'about_text', value: aboutText }); toast.success('Texte sauvegardé !'); }} className="mb-8">
                <label className="block text-lg font-semibold mb-2">Texte principal</label>
                <textarea value={aboutText} onChange={e => setAboutText(e.target.value)} rows={4} className="w-full bg-gray-800 border border-gray-700 rounded-lg text-white p-3 mb-2" placeholder="Votre texte..." />
                <button type="submit" className="px-4 py-2 bg-gold text-black rounded-lg font-semibold">Sauvegarder</button>
            </form>
            {/* My Journey */}
            <form onSubmit={async e => { e.preventDefault(); await supabase.from('settings').upsert({ key: 'about_journey', value: journey }); toast.success('Parcours sauvegardé !'); }} className="mb-8">
                <label className="block text-lg font-semibold mb-2">Section "Mon parcours"</label>
                <textarea value={journey} onChange={e => setJourney(e.target.value)} rows={4} className="w-full bg-gray-800 border border-gray-700 rounded-lg text-white p-3 mb-2" placeholder="Racontez votre histoire..." />
                <button type="submit" className="px-4 py-2 bg-gold text-black rounded-lg font-semibold">Sauvegarder</button>
            </form>
            {/* Achievements */}
            <div className="mb-10">
                ...
                <form onSubmit={e => {
                    ...
                    <div className="flex flex-col md:flex-row gap-4">
                        ...
                        <input type="text" name="title" value={achForm.title} onChange={e => setAchForm(f => ({ ...f, title: e.target.value }))} className="flex-1 bg-gray-800 border border-gray-700 rounded-lg text-white p-2" placeholder="Titre" />
                        ...
                        <button type="submit" className="px-4 py-2 bg-gold text-black rounded-lg font-semibold">{editingAchIndex !== null ? 'Modifier' : 'Ajouter'}</button>
                        {editingAchIndex !== null && <button type="button" onClick={() => { setEditingAchIndex(null); setAchForm({ image: '', title: '', desc: '' }); }} className="px-4 py-2 bg-gray-700 text-white rounded-lg">Annuler</button>}
                    </div>
                </form>
                    ...
                    {achievements.map((ach, idx) => (
                        <div key={ach.id} className="bg-gray-800 rounded-lg p-4 flex items-center justify-between shadow-lg">
                            ...
                            <div className="flex gap-2">
                                <button onClick={() => { setAchForm(ach); setEditingAchIndex(idx); }} className="px-2 py-1 bg-gold text-black rounded">Modifier</button>
                                <button onClick={() => { setAchievements(achievements.filter((_, i) => i !== idx)); setEditingAchIndex(null); setAchForm({ image: '', title: '', desc: '' }); }} className="px-2 py-1 bg-red-600 text-white rounded">Supprimer</button>
                ...
                <button onClick={async () => { await supabase.from('settings').upsert({ key: 'about_achievements', value: JSON.stringify(achievements) }); toast.success('Achievements sauvegardés !'); }} className="px-8 py-3 bg-gold hover:bg-gold/90 text-black font-semibold rounded-lg transition-all duration-200">Sauvegarder les achievements</button>
            </div>
            {/* Specialties */}
            <div className="mb-10">
                ...
                <form onSubmit={e => {
                    ...
                    <div className="flex flex-col md:flex-row gap-4">
                        ...
                        <input type="text" name="title" value={specForm.title} onChange={e => setSpecForm(f => ({ ...f, title: e.target.value }))} className="flex-1 bg-gray-800 border border-gray-700 rounded-lg text-white p-2" placeholder="Titre" />
                        ...
                        <button type="submit" className="px-4 py-2 bg-gold text-black rounded-lg font-semibold">{editingSpecIndex !== null ? 'Modifier' : 'Ajouter'}</button>
                        {editingSpecIndex !== null && <button type="button" onClick={() => { setEditingSpecIndex(null); setSpecForm({ image: '', title: '', desc: '' }); }} className="px-4 py-2 bg-gray-700 text-white rounded-lg">Annuler</button>}
                    </div>
                </form>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    {specialties.map((spec, idx) => (
                        <div key={spec.id} className="bg-gray-800 rounded-lg p-4 flex items-center justify-between shadow-lg">
                            ...
                            <div className="flex gap-2">
                                <button onClick={() => { setSpecForm(spec); setEditingSpecIndex(idx); }} className="px-2 py-1 bg-gold text-black rounded">Modifier</button>
                                <button onClick={() => { setSpecialties(specialties.filter((_, i) => i !== idx)); setEditingSpecIndex(null); setSpecForm({ image: '', title: '', desc: '' }); }} className="px-2 py-1 bg-red-600 text-white rounded">Supprimer</button>
                ...
                <button onClick={async () => { await supabase.from('settings').upsert({ key: 'about_specialties', value: JSON.stringify(specialties) }); toast.success('Specialties sauvegardées !'); }} className="px-8 py-3 bg-gold hover:bg-gold/90 text-black font-semibold rounded-lg transition-all duration-200">Sauvegarder les specialties</button>
            </div>
            {/* Stats */}
            <div className="mb-10">
                ...
                <form onSubmit={e => {
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                        ...
                        <input type="text" name="label" value={statForm.label} onChange={e => setStatForm(f => ({ ...f, label: e.target.value }))} className="bg-gray-800 border border-gray-700 rounded-lg text-white p-2" placeholder="Titre" />
                    ...
                    <div className="flex gap-2">
                        <button type="submit" className="px-6 py-2 bg-gold text-black rounded-lg font-semibold">{editingStatIndex !== null ? 'Modifier' : 'Ajouter'}</button>
                        {editingStatIndex !== null && <button type="button" onClick={() => { setEditingStatIndex(null); setStatForm({ icon: 'Camera', number: '', label: '', color: '#fff', size: '2rem' }); }} className="px-4 py-2 bg-gray-700 text-white rounded-lg">Annuler</button>}
                    </div>
                </form>
                        ...
                        return (
                            <div key={stat.id} className="bg-gray-800 rounded-lg p-4 flex flex-col items-center relative shadow-lg">
                                ...
                                <div className="flex gap-2 mt-2">
                                    <button onClick={() => { setStatForm(stat); setEditingStatIndex(idx); }} className="px-2 py-1 bg-gold text-black rounded">Modifier</button>
                                    <button onClick={() => { setAboutStats(aboutStats.filter((_, i) => i !== idx)); setEditingStatIndex(null); setStatForm({ icon: 'Camera', number: '', label: '', color: '#fff', size: '2rem' }); }} className="px-2 py-1 bg-red-600 text-white rounded">Supprimer</button>
                ...
                <button onClick={async () => { await supabase.from('settings').upsert({ key: 'about_stats', value: JSON.stringify(aboutStats) }); toast.success('Stats sauvegardées !'); }} className="px-8 py-3 bg-gold hover:bg-gold/90 text-black font-semibold rounded-lg transition-all duration-200">Sauvegarder les stats</button>
            </div>
            {/* CTA */}
            <div className="mb-10">
                ...
                <form onSubmit={async e => {
                    ...
                    ]); toast.success('Bloc CTA sauvegardé !');
                }} className="space-y-4 mb-6">
                    <div>
                        <label className="block text-sm font-medium text-gray-300 mb-2">Titre</label>
                        <input type="text" value={cta.title} onChange={e => setCta(f => ({ ...f, title: e.target.value }))} className="w-full bg-gray-800 border border-gray-700 rounded-lg text-white p-2" placeholder="Titre du bloc" />
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <textarea value={cta.desc} onChange={e => setCta(f => ({ ...f, desc: e.target.value }))} rows={2} className="w-full bg-gray-800 border border-gray-700 rounded-lg text-white p-2" placeholder="Description du bloc" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-300 mb-2">Texte du bouton</label>
                        <input type="text" value={cta.btn} onChange={e => setCta(f => ({ ...f, btn: e.target.value }))} className="w-full bg-gray-800 border border-gray-700 rounded-lg text-white p-2" placeholder="Texte du bouton" />
                    </div>
                    <div>
